...
def translate(file: list[str], from_code: str, to_code: str):
    ...
    for key in data:
        ...
        if prob >= 0.75:
            ...
            print(f"::error file=translations/app_es.arb,line={line}::Very likely offensive language")
        elif prob >= 0.5:
            line = find_line(file, data[key])
            print(f"::warning file=translations/app_es.arb,line={line}::Likely offensive language")
        elif prob >= 0.1:
            line = find_line(file, data[key])
            print(f"::notice file=translations/app_es.arb,line={line}::Potentially offensive language")
